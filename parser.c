...
static int parse_range_u16(const char *s, struct rte_acl_field *out);
static int parse_range_u8(const char *s, struct rte_acl_field *out);
...
bool parse_ruleset(ruleset_t *ruleset, const char *file) {
    ...
    while(fgets(inp_line, PARSER_LINESIZE, fd)!=NULL) {
        ...
        if(sscanf(inp_line, "%hhu.%hhu.%hhu.%hhu/%hhu %hhu.%hhu.%hhu.%hhu/%hhu %12s %12s %12s %6s",
			...
			if(parse_range_u8(v_buf[2], ruleset->rules[ruleset->num_rules]->field_value)) {
            ...
            for(int i=0; i<2; ++i) {
                if(parse_range_u16(v_buf[i], ruleset->rules[ruleset->num_rules]->field_value+3+i)) {
                    fprintf(stderr, "ERROR: could not parse range: \"%s\"\n", v_buf[i]);
                    goto failure;
                }
            }
